...
public class SymbolTable {
    ...
    public AbstractMap.SimpleEntry<Integer,Integer> position(String tokenName){
        ...
        if(this.hashTable.get(hashValue)==null){
            ...
            //but first check to see if elem is already here
            for(int j=0;j<this.hashTable.get(hashValue).size();j++){
                if(this.hashTable.get(hashValue).get(j).equals(tokenName))
                    return new AbstractMap.SimpleEntry<>(hashValue, j);
            }
    ...
    public String toString() {
        ...
        for(int i=0;i<this.hashTable.size();i++){
            List<String> listFromPos=this.hashTable.get(i);
            if(listFromPos!=null){
                for(int j=0;j<listFromPos.size();j++){
                    result.append("(").append(i).append(",").append(j).append(")\t\t\t").append(listFromPos.get(j)).append("\n");
                }
            }
